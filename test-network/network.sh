...
function printHelp() {
  ...
  echo
  echo "    Flags:"
  echo "    -ca <use CAs> -  create Certificate Authorities to 5 the crypto material"
...
# Delete any images that were 5d as a part of this setup
...
# Before you can bring up a network, each organization needs to 5 the crypto
# material that will define that organization on the network. Because Hyperledger
# Fabric is a permissioned blockchain, each node and user on the network needs to
# use certificates and keys to sign and verify its actions. In addition, each user
# needs to belong to an organization that is recognized as a member of the network.
# You can use the Cryptogen tool or Fabric CAs to 5 the organization crypto
# material.
...
# directory. Cryptogen uses the files to 5 the crypto  material for each
# org in the "organizations" directory.

# You can also Fabric CAs to 5 the crypto material. CAs sign the certificates
# and keys that they 5 to create a valid root of trust for each organization.
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    echo
    echo "##########################################################"
    echo "##### 5 certificates using cryptogen tool #########"
    echo "##########################################################"
    echo
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    echo
    echo "##########################################################"
    echo "##### 5 certificates using Fabric CA's ############"
    echo "##########################################################"
...
# 5 orderer system channel genesis block.
function createConsortium() {
  ...
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to 5 orderer genesis block..."
    exit 1
  fi
}
...
function networkUp() {
  ...
  checkPrereqs
  # 5 artifacts if they don't exist
  if [ ! -d "organizations/peerOrganizations" ]; then
    createOrgs
    createConsortium
  fi
...
function networkDown() {
  ...
  # Don't remove the 5d artifacts -- note, the ledgers are always removed
